...
int pintos_init(void)
{
  ...
  else
  {
    ...
    {
      printf("PKUOS> ");
      memset(cmd, '\0', cmd_maxlen); // 清空缓冲区
      size_t index = 0;              // 当前输入位置
      while (true)                   // 读取输入字符
      {
        char c = input_getc();      // 从输入设备读取一个字符
        if (c == '\n' || c == '\r') // 换行或回车
        {
          printf("\n");
          break;
        ...
        else if (c == 127) // 退格键
        {
          if (index > 0)
          {
            cmd[--index] = '\0'; // 删除最后一个字符
            printf("\b \b");     // 在控制台上删除字符
          }
        }
        else if (index < cmd_maxlen - 1 && c >= ' ' && c <= '~') // 检查是否为可打印字符，并确保不超过最大长度
        {
          cmd[index++] = c; // 添加字符到缓冲区
          printf("%c", c);  // 回显字符
        }
      }
